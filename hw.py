...
def missing_elements(my_list: list) -> list:
    ...
    for i in range(len(my_list) - 1):
        ...
        if diff > 1:
            l.extend(range(my_list[i] + 1, my_list[i + 1]))
    return l
...
def swap_dict(d: dict) -> dict:
    keys1 = d.keys()
    values1 = d.values()
    dict2 = {value: key for key, value in zip(keys1, values1)}
    return dict2
...
def most_frequent(my_list: list) -> int:
    ...
    most_frequent_index = counters.index(max_count)
    return unique_list[most_frequent_index]
